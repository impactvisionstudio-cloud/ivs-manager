...
function tryParseResult(content: string): AiResult | null {
  try {
    const parsed = JSON.parse(content);
    if (parsed && parsed.ready === true) return parsed as AiResult;
    return null;
  } catch {
    return null;
  }
  ...
  const openConversation = async (id: string) => {
    ...
    try {
      ...
      const loaded: Message[] = data.map((m) => {
        if (m.role === "assistant") {
          const result = tryParseResult(m.content);
          if (result) {
            return {
              id: m.id,
              role: "assistant",
              content: `Aqui está a análise completa para ${result.niche}:`,
              result,
            };
          }
        }
            ...
            <div className="scrollbar-thin flex-1 space-y-1 overflow-y-auto p-2">
              {conversations.length === 0 && (
                <p className="p-2 text-xs text-muted-foreground">
                  Suas análises anteriores vão aparecer aqui.
                </p>
              )}
              {conversations.map((c) => (
                <button
